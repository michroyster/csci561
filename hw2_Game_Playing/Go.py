...
class GO:
    ...
    def future_boards(self, stone):
        """ Generates all valid future states
            stone: generate moves for this color of stone
            returns list of tuples: (heuristic, board, move)
        """
        future_boards = []
        for i in range(SIZE):
            for j in range(SIZE):
                if self.valid_place_check(i, j, stone):                  
                    new_board = self.future_board(i, j, stone)
                    if new_board != None:
                        future_boards.append((new_board, (i,j)))
    ...
    def future_board(self, i, j, stone):
        copy_board = self.copy_board()
        if not copy_board.place_chess(i, j, stone):
            return None
        copy_board.remove_died_pieces(3-stone)
        return copy_board
    ...
    def evaluate_heuristic(self, board):
        ...
        black_liberties, white_liberties = 0, 0
        for i in range(SIZE):
            for j in range(SIZE):
                if board[i][j] == 1:
                    black_stones += 1
                    ...
                    black_liberties += self.single_liberty_check(board, i, j, BLACK)
                    white_liberties += self.single_liberty_check(board, i, j, WHITE)
        stones = black_stones - white_stones
        liberties = black_liberties - white_liberties
        return stones + liberties
    
    def single_liberty_check(self, board, i, j, color):
...
def main():
    ...
    go = GO(SIZE)
    go.set_board(stone, previous_board, board)
    go.visualize_board()
    go.place_chess(3,2,2)
    go.visualize_board()
